  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://codix-test.vercel.app/api/create-portfolio', {
        ...
        body: JSON.stringify({ title, content,coverImage,description }),
      })
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="w-full bg-white shadow-xl">
        <div className="bg-[#0028ff] p-4 sm:p-6 text-white">
          ...
          <p className="mt-2 text-purple-100 text-center sm:text-left">Share your Projects with the world</p>
        </div>
        
        <form onSubmit={handleSubmit} className="p-4 sm:p-8 overflow-y-auto">
          <div className="mb-6">
            <label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-2">Portfolio Title</label>
            <input 
              type="text" 
              id="title"
              value={title}
